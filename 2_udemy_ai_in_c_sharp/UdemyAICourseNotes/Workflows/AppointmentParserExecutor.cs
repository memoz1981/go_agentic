...
internal class AppointmentParserExecutor(AIAgent agent) : Executor<string, AppointmentDto>("AppointmentParser")
{
...
18 for 18am, 0 for midnight etc. Parse the user data - provide one AppointmentDto object, use DateTime for dates
...
put all info into description. If there's a name - parse it to name property. input data is start only most of the time. "; 
    ...
    public override async ValueTask<AppointmentDto> HandleAsync(
        string message, 
        IWorkflowContext context, 
        CancellationToken cancellationToken = default)
    {
        GrayLine("Appointment parser parsing message...");
        var response = await agent.RunAsync<AppointmentDto>($"{INSTRUCTIONS}: {message}");
